...
function getEscrowChat(req, res) {
  try {
    const db = readDB();
    const user = getUserById(db, req.user.id);
    const tx = (db.transactions || []).find(t => t.id === req.params.txId);
...
function getDirectChats(req, res) {
  try {
    const db = readDB();
    const user = getUserById(db, req.user.id);
...
function getDirectChat(req, res) {
  try {
    const db = readDB();
    const user = getUserById(db, req.user.id);
...
function postDirectMessage(req, res) {
  try {
    const { text } = req.body;
    if (!text || text.trim().length === 0) return res.status(400).json({ success: false, error: 'Message text required' });
    if (containsProfanity(text)) return res.status(400).json({ success: false, error: 'Message contains forbidden language' });
...
function getSupportChats(req, res) {
  try {
    const db = readDB();
    const user = db.users.find(u => u.id === req.user.id);
    if (!user || (user.role !== 'admin' && user.role !== 'escrow')) {
      return res.status(403).json({ success: false, error: 'Only admin and escrow can view all support chats' });
    }
...
function getSupportChat(req, res) {
  try {
    const db = readDB();
    const user = db.users.find(u => u.id === req.user.id);
...
function createSupportChat(req, res) {
  try {
    const db = readDB();
    const user = db.users.find(u => u.id === req.user.id);
    if (!user) return res.status(401).json({ success: false, error: 'Unauthorized' });
...
function postSupportMessage(req, res) {
  try {
    const { text } = req.body;
    if (!text || text.trim().length === 0) return res.status(400).json({ success: false, error: 'Message text required' });
    if (containsProfanity(text)) return res.status(400).json({ success: false, error: 'Message contains forbidden language' });
...
function banSupportChat(req, res) {
  try {
    const db = readDB();
    const user = db.users.find(u => u.id === req.user.id);
    if (!user || (user.role !== 'admin' && user.role !== 'escrow')) {
...
function assignSupportChat(req, res) {
  try {
    const db = readDB();
    const user = db.users.find(u => u.id === req.user.id);
