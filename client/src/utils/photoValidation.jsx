...
export async function validatePhoto(file) {
  ...
  // Check file type
  if (!PHOTO_CONSTRAINTS.ALLOWED_FORMATS.includes(file.type)) {
    errors.push(`File type not supported. Allowed: JPEG, PNG, WebP (current: ${file.type})`);
  }

  // Check file extension
  const ext = file.name.substring(file.name.lastIndexOf('.')).toLowerCase();
  if (!PHOTO_CONSTRAINTS.ALLOWED_EXTENSIONS.includes(ext)) {
