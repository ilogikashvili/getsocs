...
test('renders cart data from localStorage', async () => {
  window.localStorage.setItem('cart', JSON.stringify([
    { id: 1, title: 'Demo account', price: 150, platform: 'YouTube', images: [] }
  ]));
...
test('filters listings by a minimum and maximum price range', async () => {
  getProducts.mockResolvedValue({
    data: {
      data: [
        { id: 1, title: 'Budget account', price: 50, platform: 'YouTube', topic: 'Gaming', followers: 1000, monetized: false },
        { id: 2, title: 'Premium account', price: 120, platform: 'YouTube', topic: 'Gaming', followers: 1000, monetized: true }
      ]
    }
  });
  ...
  const premiumItems = await screen.findAllByText(/Premium account/i);
