...
import { getProduct, getProducts, commentProduct, editComment, deleteComment, buyProduct, deleteProduct, claimProduct, submitProductClaim } from '../services/productService';
...
export default function ProductDetail() {
  ...
  async function handleBuy() {
    ...
    try {
      ...
      const tx = res.data.tx || res.data.transaction || res.data.data;
      if (res.data.success && tx?.id) {
        navigate('/messages', { state: { openTransactionId: tx.id } });
      } else {
        setError(res.data.error || 'Unable to start purchase chat');
      }
    } catch (e) {
      setError(e.response?.data?.error || 'Unable to start purchase chat');
    } finally {
      setBuying(false);
    }
  }
  ...
  async function saveCommentEdit(commentId) {
    ...
    try {
      const res = await editComment(id, commentId, editingCommentText.trim());
      if (res.data.success) {
        setProduct(prev => ({
          ...prev,
          comments: prev.comments.map(c => c.id === commentId ? { ...c, text: editingCommentText.trim(), editedAt: new Date().toISOString() } : c)
        }));
        setEditingCommentId(null);
        setEditingCommentText('');
      } else {
        setError(res.data.error || 'Could not update comment');
      }
    } catch (e) {
      setError(e.response?.data?.error || 'Could not update comment');
    }
  }
  ...
  async function removeComment(c) {
    ...
    try {
      const res = await deleteComment(id, c.id);
      if (res.data.success) {
        setProduct(prev => ({ ...prev, comments: prev.comments.filter(item => item.id !== c.id) }));
      } else {
        setError(res.data.error || 'Could not delete comment');
      }
    } catch (e) {
  ...
  const isYouTubeProduct = product.platform === 'YouTube';
      ...
      <section className="product-deal-layout">
        ...
        <div className="product-info-panel">
          ...
          <div className="product-metrics">
            ...
            {isYouTubeProduct && <div><strong>{product.monetized ? 'Yes' : 'No'}</strong><span>monetized</span></div>}
          </div>
          ...
          <div className="product-price-row">
            ...
            <div className="product-action-row">
              <button className="btn primary" type="button" onClick={handleBuy} disabled={buying}>
                {buying ? 'Opening chat...' : 'Buy this account'}
              </button>
              <button
        ...
        <aside className="seller-detail-card">
          ...
          <button className="btn primary" type="button" onClick={handleBuy} disabled={buying}>
            {buying ? 'Opening chat...' : 'Contact seller'}
          </button>
          <button
      ...
      <section className="product-lower-grid">
        <div className="section-card">
          ...
          <div className="product-description-list">
            ...
            {isYouTubeProduct && <div><span>Revenue state</span><strong>{product.monetized ? 'Monetized' : 'Unmonetized'}</strong></div>}
          </div>
        </div>
                ...
                return (
                  <article key={c.id} className="comment-row">
                    ...
                    <div className="comment-content">
                      <div className="comment-meta comment-meta-with-actions">
                        <strong>{commentName}</strong>
                        <small>{new Date(c.ts).toLocaleString()}</small>
                        {(isOwner || isModerator) && (
                          <span className="comment-actions">
                            {isOwner && !isEditing && (
                              <button type="button" className="link-btn" onClick={() => { setEditingCommentId(c.id); setEditingCommentText(c.text); }}>
                                <Edit3 size={14} /> Edit
                              </button>
                            )}
                            <button type="button" className="link-btn" onClick={() => removeComment(c)}>
                      ...
                      {isEditing ? (
                        <div className="comment-edit-form">
                          ...
                          <div className="comment-edit-actions">
                            <button type="button" className="btn secondary" onClick={() => { setEditingCommentId(null); setEditingCommentText(''); }}>
                              Cancel
                            </button>
                            <button type="button" className="btn primary" onClick={() => saveCommentEdit(c.id)} disabled={!editingCommentText.trim()}>
                              Save
                            </button>
                          </div>
                        </div>
