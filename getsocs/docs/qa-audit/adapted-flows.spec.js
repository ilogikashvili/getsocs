...
const DB_FILE = path.resolve(__dirname, 'audit.test.json');
const IMAGE_FILE = path.resolve(__dirname, '../../e2e/fixtures/test-image.png');
const PRODUCT_IMAGES = [IMAGE_FILE, path.resolve(__dirname, '../../e2e/fixtures/test-image-2.png'), path.resolve(__dirname, '../../e2e/fixtures/test-image-3.png')];
...
async function registerVerified(request, label, role = 'user') {
  ...
  const registration = await request.post(`${API}/auth/register`, {
    multipart: { username, name: 'Audit', lastname: label, email, password: PASSWORD }
  });
...
test.describe('Getsocs critical end-to-end journeys', () => {
  ...
  test('profile photo traverses browser multipart -> backend -> public upload route', async ({ page, request }) => {
    ...
    const imageResponse = await request.get(`http://127.0.0.1:3101/uploads/${updated.profilePhoto}`);
