...
const youtubeBreaker = new CircuitBreaker('youtube-api', { failureThreshold: 4, resetTimeoutMs: 30_000 });
...
async function fetchYoutubeJson(url) {
  return youtubeBreaker.execute(async () => {
...
async function resolveChannelId(ref, apiKey = YOUTUBE_API_KEY) {
  ...
  if (ref.type === 'handle') {
    const json = await fetchYoutubeJson(`${YOUTUBE_API_BASE}/channels?part=id&forHandle=${encodeURIComponent(ref.value)}&key=${apiKey}`);
    if (json.items && json.items.length) return json.items[0].id;
  }

  if (ref.type === 'user') {
    const json = await fetchYoutubeJson(`${YOUTUBE_API_BASE}/channels?part=id&forUsername=${encodeURIComponent(ref.value)}&key=${apiKey}`);
    if (json.items && json.items.length) return json.items[0].id;
  }

  // Fall back to a general search for custom URLs / anything unresolved above
  const searchJson = await fetchYoutubeJson(`${YOUTUBE_API_BASE}/search?part=snippet&type=channel&q=${encodeURIComponent(ref.value)}&maxResults=1&key=${apiKey}`);
...
async function getYoutubeChannelMetadata(url, verificationCode, apiKey = YOUTUBE_API_KEY) {
  ...
  try {
    ...
    const channelJson = await fetchYoutubeJson(`${YOUTUBE_API_BASE}/channels?part=snippet,statistics,contentDetails&id=${channelId}&key=${apiKey}`);
    ...
    if (uploadsPlaylistId) {
      const playlistJson = await fetchYoutubeJson(`${YOUTUBE_API_BASE}/playlistItems?part=contentDetails&playlistId=${uploadsPlaylistId}&maxResults=10&key=${apiKey}`);
      const videoIds = (playlistJson.items || []).map(item => item.contentDetails.videoId).filter(Boolean);
      if (videoIds.length) {
        const videosJson = await fetchYoutubeJson(`${YOUTUBE_API_BASE}/videos?part=statistics&id=${videoIds.join(',')}&key=${apiKey}`);
    ...
    logger.error('YouTube verification error', { error: e, circuit: youtubeBreaker.snapshot() });
    return { ok: false, error: 'Could not reach YouTube to verify this channel right now. Please try again.' };
  }
}
